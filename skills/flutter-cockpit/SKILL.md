...
- Evidence: key mutating commands auto-attach best-effort screenshots. Use explicit `captureScreenshot` for final still proof. Use `--recording-json` only for motion, transition, or acceptance video, then verify a completed non-empty artifact.
...
- Persistent sessions: `launch-development-session` -> `collect-development-probe --profile quick` -> edit -> `reload-development-session --mode hot_reload` -> collect/compare probe. Stop only for cleanup or recovery.

...
- For route-changing taps, include `parameters.expectedRouteName`. Auto activation uses it to verify the direct/semantic path quickly, fall back to a user-like gesture when safe, and return better diagnostics. `timeoutMs` is only the hard command ceiling; add `parameters.routeTimeoutMs` only when the route is intentionally slow.

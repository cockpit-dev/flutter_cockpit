...
下面，生产 `lib/` 代码和生产入口保持不变。

iOS 和 macOS 同时支持 CocoaPods 与 Swift Package Manager。插件在两个平台
都提供 `.podspec` 和 `Package.swift`，Flutter 会按照宿主工程选择的集成方式
自动接入。现有 CocoaPods 工程无需开启 SwiftPM，SwiftPM 工程也不需要 Podfile。
...
- [`packages/cockpit_protocol`](packages/cockpit_protocol)：纯 Dart 的 V2 用例、运行结果、错误和报告包协议
...
需要完整观测交付数据时，对 `run-script`、`run-task` 或 `validate-task` 使用的
同一个 output root 启动无头 loopback API：
...
它不内置 HTML 或 Flutter UI。CLI/MCP 仍是内置入口；未来 Flutter GUI 或三方客户端
独立消费同一套 `/api/*` 认证接口。生成的 `report.html` 是可携带的回归报告产物，
不依赖该服务。
...
生产安全的开发接入使用独立的 `cockpit/` Flutter shell。显式传入
`--project-dir cockpit --target main.dart`，入口只在 shell 项目中解析，不改动正式应用。
...
创建独立的 `cockpit/` 开发项目并在其 `dev_dependencies` 中加入
`flutter_cockpit` 和 `cockpit`，入口为 `main.dart`。保持正常生产入口、生产
`lib/` 以及正式发布依赖图不变，不要把 `flutter_cockpit` import 加到生产 `lib/` 代码里：

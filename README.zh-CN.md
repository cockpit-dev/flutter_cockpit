...
- Android 与 iOS 的应用内原生验收录屏，以及 macOS / Windows / Linux 的 plugin-native 桌面录屏，并将视频写入标准 task-run bundle
...
- Android、iOS、macOS、Windows 与 Linux 上的验收截图能力：移动端可通过 `flutter_cockpit` plugin bridge 走原生验收截图；桌面端默认走 plugin 内原生窗口截图
- Android 与 iOS 上通过 `flutter_cockpit` plugin bridge 的原生验收录屏，以及 macOS / Windows / Linux 的 plugin-native 桌面录屏
...
- Windows 且使用 `--platform windows`：默认走 `flutter_cockpit` plugin 内的原生窗口截图/录屏；只有在 native 产物链不可用时才回退到 remote screenshot 或基于截图时间线合成的交付视频
- Linux 且使用 `--platform linux`：默认走 `flutter_cockpit` plugin 内的原生窗口截图/录屏；只有在 native 产物链不可用时才回退到 remote screenshot 或基于截图时间线合成的交付视频
...
- 已支持的录屏路径：应用内 native acceptance recording，加上 Android emulator / iOS Simulator 的 host-side remote fallback，以及 macOS / Windows / Linux 的 plugin-native 桌面录屏；当平台录屏无法 finalize 时，bundle writer 会用截图时间线合成有界交付视频

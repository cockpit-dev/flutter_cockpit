...
- Android、iOS、macOS、Windows 与 Linux 上的验收截图能力：移动端可通过 `flutter_cockpit` plugin bridge 走原生验收截图；macOS 与 Windows 默认走 plugin 内原生窗口截图，Linux 仍优先走 host 侧截图适配器
...
- 已支持的录屏路径：应用内 native acceptance recording，加上 Android emulator / iOS Simulator / macOS / Windows 的 plugin-native 桌面录屏，以及 Linux 的 host-side fallback；当平台录屏无法 finalize 时，bundle writer 会用截图时间线合成有界交付视频

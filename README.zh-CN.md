...
会切换路由的 `tap` 应附带 `parameters.expectedRouteName`；在 CI、录屏、模拟器等延迟可预期的验收流程中再加 `parameters.routeTimeoutMs`。`timeoutMs` 是命令的硬上限，不是默认路由等待。关键路由切换后，再补一个针对 `parameters.routeName` 的 `waitFor`。

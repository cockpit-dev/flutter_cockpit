...
如果后续几步已经明确，而且流程会跨路由，比如 `/inbox -> /editor -> /inbox`，优先把它们收成一次有序的 `run-batch`，不要拆成多次 `run-command` 往返。这样更省 token，也更能避开路由切换窗口期的状态抖动。
如果 app summary 已经暴露了 `syncStatus`、`pendingTaskCount`、`failedTaskCount`、`conflictTaskCount` 这类工作流计数字段，优先先读这些 bounded summary，再决定是否打开更重的 inspect 载荷。
...
`examples/cockpit_demo` 现在内置了一个仓库自维护的 Sync Lab 流程，用来验证本地修改进入待同步队列、混合同步结果、仅冲突项过滤、产品内冲突解决，以及解决后的重试同步。

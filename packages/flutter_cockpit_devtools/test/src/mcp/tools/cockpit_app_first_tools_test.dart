...
void main() {
  test('MCP integer arguments reject fractional numbers instead of truncating',
      () {
    expect(
      () => cockpitReadOptionalInt(
        const <String, Object?>{'timeoutMs': 1.5},
        'timeoutMs',
      ),
      throwsA(
        isA<CockpitMcpError>()
  ...
  test('MCP positive integer arguments reject zero and negative values', () {
    expect(
      () => cockpitReadOptionalPositiveInt(
        const <String, Object?>{'timeoutMs': 0},
        'timeoutMs',
      ),
      throwsA(
        isA<CockpitMcpError>()
    ...
    expect(
      () => cockpitReadOptionalPositiveInt(
        const <String, Object?>{'maxLines': -1},
        'maxLines',
      ),
  ...
  test('MCP HTTP status arguments reject values outside the HTTP range', () {
    expect(
      () => cockpitReadOptionalHttpStatusCode(
        const <String, Object?>{'statusCodeAtLeast': 99},
        'statusCodeAtLeast',
      ),
      throwsA(isA<CockpitMcpError>()),
    );
    ...
    expect(
      () => cockpitReadOptionalHttpStatusCode(
        const <String, Object?>{'statusCodeAtLeast': 600},
        'statusCodeAtLeast',
      ),
      throwsA(isA<CockpitMcpError>()),
    );
  ...
  test('MCP port arguments reject values outside the TCP range', () {
    expect(
      () => cockpitReadRequiredPort(
        const <String, Object?>{'sessionPort': 0},
        'sessionPort',
      ),
      throwsA(isA<CockpitMcpError>()),
    );
    ...
    expect(
      () => cockpitReadRequiredPort(
        const <String, Object?>{'sessionPort': 65536},
        'sessionPort',
      ),
      throwsA(isA<CockpitMcpError>()),
    );

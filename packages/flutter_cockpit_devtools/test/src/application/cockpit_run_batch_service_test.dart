...
void main() {
  group('CockpitRunBatchService', () {
    test('wraps app-first batch recording with app-aware recording services',
        () async {
      ...
      expect(result.recordingSession?.recordingSession.state,
      ...
      expect(result.results.single.sessionHandle?.toJson(),
          _remoteSessionHandle().toJson());
    });

    test('passes explicit iOS device id through batch recording wrappers',
        () async {
      CockpitStartRecordingRequest? capturedStart;
      CockpitStopRecordingRequest? capturedStop;
      final service = CockpitRunBatchService(
      ...
      expect(
          capturedStart?.iosDeviceId, '6FD25DED-11E9-4AE9-B4B5-EDF4601981DC');
      expect(capturedStop?.iosDeviceId, '6FD25DED-11E9-4AE9-B4B5-EDF4601981DC');
    });

    test('preserves app remote session handle for non-recorded batches',
        () async {
      Uri? capturedBaseUri;
      final service = CockpitRunBatchService(
        executeService: CockpitExecuteRemoteCommandBatchService(
    ...
    test('keeps resolved route-prefixed base uri in returned session handle',
        () async {
      Uri? capturedBaseUri;
      final service = CockpitRunBatchService(
        executeService: CockpitExecuteRemoteCommandBatchService(
      ...
      expect(result.results.single.sessionHandle?.baseUrl,
    ...
    test('preserves stop failure and still retries best-effort cleanup',
        () async {
      var stopAttempts = 0;
      final service = CockpitRunBatchService(
        executeService: CockpitExecuteRemoteCommandBatchService(

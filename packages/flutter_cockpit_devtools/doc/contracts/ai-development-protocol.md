...
```bash
dart run flutter_cockpit_devtools:flutter_cockpit_devtools launch-target --target-json <file>
dart run flutter_cockpit_devtools:flutter_cockpit_devtools read-target --profile minimal
dart run flutter_cockpit_devtools:flutter_cockpit_devtools inspect-surface --profile standard
```

Use native/system control only after reading capabilities:

```bash
dart run flutter_cockpit_devtools:flutter_cockpit_devtools read-system-capabilities --platform <platform> --device-id <id>
dart run flutter_cockpit_devtools:flutter_cockpit_devtools run-system-action --platform <platform> --device-id <id> --action <available-action>
```

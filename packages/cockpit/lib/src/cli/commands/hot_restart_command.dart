...
typedef CockpitHotRestartFunction =
...
final class HotRestartCommand extends CockpitCliCommand {
  ...
  String get helpExample =>
      'cockpit hot-restart --app-json /tmp/app.json --stdout-format json | jq \'{reloadGeneration: .status.reloadGeneration, lastReloadSucceeded: .status.lastReloadSucceeded, lastReloadMode: .status.lastReloadMode}\'';
  ...
  Future<int> run() async {
    ...
    final result = await _restart(
      CockpitHotRestartRequest(appHandlePath: appJson),
    );

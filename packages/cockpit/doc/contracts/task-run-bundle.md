...
These derived objects must remain traceable back to files in the bundle, especially `manifest.json`, `handoff.json`, `delivery.json`, `acceptance.md`, `steps.json`, `trace.json`, `validation.json` when present, `observations.json`, and any referenced artifacts under `screenshots/`, `recordings/`, `keyframes/`, or `diagnostics/`.
...
- `delivery.json`
- `issue_evidence.json`
- `validation.json` after `validate-task` has checked the bundle
- `screenshots/`
- `recordings/`
...
Workflow control records use dedicated action types:
...
These records explain branch, loop, retry, and probe decisions without making failed conditions or intermediate retry attempts look like final command failures.
The input script format is defined by [`control-workflow-protocol.md`](control-workflow-protocol.md).

### `trace.json`

The trace file is a compact index derived from `steps.json`. It lets AI agents and external E2E consumers correlate workflow nodes, command ids, step indices, errors, screenshots, recordings, and diagnostics without scanning every full step payload first.
...
Each trace entry may include `stepIndex`, `actionType`, `workflowStepId`, `workflowStepType`, `commandId`, `commandType`, `status`, `conditionCommandId`, `conditionSuccess`, `selectedBranch`, `iteration`, `attempt`, `artifactRefs`, `captureRefs`, `commandError`, `conditionError`, and `failureSummary`.

`trace.json` is not a second source of truth. If a trace entry and `steps.json` disagree, `steps.json` wins and the writer or consumer should be fixed.
...
`validate-task` writes this file into the bundle after it checks the bundle. It must include:
...
This is the durable validation verdict for external automation. It links the final validation decision to the same bundle that contains `trace.json`, `steps.json`, `delivery.json`, and the artifacts.

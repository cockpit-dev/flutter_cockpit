...
import 'package:cockpit/src/application/cockpit_read_workspace_contracts_service.dart';
...
void main() {
  ...
  test('published package includes fallback contract copies', () {
    final packageRoot = Directory.current.path.endsWith('packages/cockpit')
        ? Directory.current
  ...
  test('falls back to package-local contracts when workspace docs are absent', () async {
    final fileSystem = MemoryFileSystem();
    fileSystem.file(
        '/workspace/packages/cockpit/doc/contracts/flutter-cockpit-protocol.md',
      )
      ..createSync(recursive: true)
      ..writeAsStringSync('# Package Flutter Cockpit Protocol');
    fileSystem.file(
        '/workspace/packages/cockpit/doc/contracts/ai-development-protocol.md',
      )
      ..createSync(recursive: true)
      ..writeAsStringSync('# Package AI Development Protocol');
    fileSystem.file(
        '/workspace/packages/cockpit/doc/contracts/flutter-cockpit-skill-contract.md',
      )
      ..createSync(recursive: true)
      ..writeAsStringSync('# Package Skill Contract');
    fileSystem.file(
        '/workspace/packages/cockpit/doc/contracts/task-run-bundle.md',
      )
      ..createSync(recursive: true)
      ..writeAsStringSync('# Package Bundle Contract');
    fileSystem.file(
        '/workspace/packages/cockpit/doc/contracts/control-workflow-protocol.md',
      )
      ..createSync(recursive: true)
      ..writeAsStringSync('# Package Workflow Protocol');
    fileSystem.file(
        '/workspace/packages/cockpit/doc/contracts/control-workflow.schema.json',
      )
      ..createSync(recursive: true)
